...
function refreshSellersTable() {
  const sellersTable = document
    .getElementById("sellerDataTable")
    .getElementsByTagName("tbody")[0];
  ...
  usersData.forEach((user) => {
    ...
    row.innerHTML = `
            ...
            <td>
                ${
                  user.state === false
                    ? `<button class="btn btn-outline-success btn-sm" type="button" data-id="${user.id}" data-action="activate">Activate</button>`
                    : ""
                }
                ${
                  user.state === true
                    ? `<button class="btn btn-outline-warning btn-sm" type="button" data-id="${user.id}" data-action="suspend">Suspend</button>`
                    : ""
                }
                <button class="btn btn-outline-primary btn-sm" type="button" data-id="${
                  user.id
                }" data-action="edit">Modify</button>
                <button class="btn btn-outline-warning btn-sm" type="button" data-id="${
                  user.id
                }" data-action="resetpass">Reset Password</button>
            </td>
        `;
...
function refreshSupportTable() {
  const supportTable = document
    .getElementById("supportTable")
    .getElementsByTagName("tbody")[0];
  ...
  supportData.forEach((request) => {
    ...
    row.innerHTML = `
            ...
            <td>
                ${
                  request.state === "Opened" || request.state === "Reopened"
                    ? `<button class="btn btn-outline-success btn-sm" type="button" data-id="${request.id}" data-action="resolve">Resolve</button>`
                    : ""
                }
            </td>
        `;
...
function refreshRejectedSellerTable() {
  const rejectedSellerRequestsTable = document
    .getElementById("RejectedSellerRequests")
    .getElementsByTagName("tbody")[0];
...
function renderSellerRequestsTable() {
  const sellerRequestsTable = document
    .getElementById("sellerRequestsTable")
    .getElementsByTagName("tbody")[0];
...
document.addEventListener("DOMContentLoaded", function () {
  if (
    window.location.pathname === "/Views/adminPanel.html" ||
    window.location.pathname === "/views/adminpanel"
  ) {
    ...
    document
      ...
      .addEventListener("submit", handleEditSellerFormSubmit);
    document
      .getElementById("editUserForm")
      .addEventListener("submit", handleEditUserFormSubmit);
    document
      .getElementById("editProductModal")
      .addEventListener("submit", handleEditProductFormSubmit);
...
function handleSupportRequestAction(event) {
  ...
  if (action === "resolve") {
    ...
    const modal = new bootstrap.Modal(
      document.getElementById("Resolve_Reason")
    );
    modal.show();
    ...
    if (confirmButton) {
      confirmButton.addEventListener(
        "click",
        function () {
          const resolveRemarks = document.getElementById(
            "Resolve_Reasoninput"
          ).value;
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    .getElementById("sellerRequestsTable")
    .addEventListener("click", handleSellerRequestAction);
  document
    .getElementById("RejectedSellerRequests")
    .addEventListener("click", handleRejectedSellerRequests);
  document
    .getElementById("sellerDataTable")
    .addEventListener("click", handleSellerAction);
  document
    .getElementById("usersTable")
    .addEventListener("click", handleUserAction);
  document
    .getElementById("productsTable")
    .addEventListener("click", handleProductsAction);
  document
    .getElementById("supportTable")
    .addEventListener("click", handleSupportRequestAction);
});
